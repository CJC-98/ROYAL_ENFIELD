...
public class DealerController {
	...
	//<!-- this method to save form data to db @author Akshata Yevatkar
	@RequestMapping("/save")
	public String saveData(@RequestParam String rolename,@RequestParam String email,@ModelAttribute Registration r,@ModelAttribute Login l,@ModelAttribute Contact c,@ModelAttribute Role ro,@ModelAttribute Address a,Model model)
	{
		Date dt=new Date();
	    int d=dt.getDate();
	    int m=dt.getMonth()+1;
		...
		else
		{
			if(rolename.equals("DEALER"))
			{
				...
				Login lgn2=service.findEmail(email);
				
				System.out.println("home controller emails"+lgn);
				if(lgn!=null){
					System.out.println("in else if email");
					String msg="email already exist...please enter another email";
					model.addAttribute("msg",msg );
					return "signup";
