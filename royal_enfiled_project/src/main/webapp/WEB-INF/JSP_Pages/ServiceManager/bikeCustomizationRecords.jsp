...
<meta charset="utf-8" />
	...
	<script>
		function releaseBike(custId) {
			
			var req = new XMLHttpRequest();		
			req.open("GET","releaseCustomizedBike?custId="+custId,true);
			req.send(); 
                                        ...
                                        <table class="table table-striped m-b-none b-a">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="text-align: center">Customization Status</th>
                                            ...
                                            <tbody>
                                              <a:forEach var="data" items="${bikeCustomizationsList}">
                                                <tr style="text-align: center">
                                                         ...
                                                         <td>${data.customizationBikeInfo.plateNumber}</td>
                                                         <td>${data.customizationBikeInfo.chasisNumber}</td>
                                                         
                                                         <a:choose>
															  <a:when test="${data.customizationStatus == 'waiting'}">
															  		<td style="color: #FF681B">waiting</td>
															  </a:when>
															  <a:when test="${data.customizationStatus == 'in-progress'}">
														...
														<a:choose>
															  <a:when test="${data.bikeReleaseStatus == 'released' || data.customizationProgressPercent != '100'}">
															  		<td> 
															  ...
															  <a:otherwise>
															        <td> 
			                                                            <input type="button" class="btn btn-md btn-success btn-rounded" id="myBtn" onclick="releaseBike('${data.bikeCustomizationId}')" value="Release Bike"/>
			                                                        </td>
															  </a:otherwise>
														</a:choose>
